...
export async function runAudit(auditId, targetUrl) {
  ...
  try {
    ...
    Solari = mod.Solari || mod.default?.Solari || mod.default;
    if (!Solari) throw new Error("Solari class export not found");
  } catch (err) {
    console.warn("[Solari Agent] Native browser SDK unavailable on serverless platform:", err.message);
    return runServerlessDomAudit(auditId, targetUrl, "Serverless environment cannot load native Playwright browser binaries.");
  }
